...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans selection:bg-primary selection:text-white">
      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 py-6 px-6 md:px-12 ${scrolled ? 'glass py-4 shadow-sm' : ''}`}>
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => scrollTo('hero')}>
            <Logo 
              size={48} 
              color={scrolled ? "#C05A3F" : "#FFFFFF"} 
              className="group-hover:rotate-12 transition-transform duration-500"
            />
            <span className={`text-2xl font-heading font-extrabold tracking-tight uppercase transition-colors ${scrolled ? 'text-charcoal' : 'text-white'}`}>Spirited Lamb</span>
          </div>
        ...
        {/* Mobile Nav */}
        {isMenuOpen && (
          <div className="absolute top-full left-0 right-0 glass mt-4 mx-4 p-8 rounded-3xl space-y-6 flex flex-col items-center animate-in fade-in slide-in-from-top-4 duration-300">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollTo(item.id)}
      ...
      <footer className="bg-charcoal text-background py-16 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-12">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <Logo size={40} color="#FAF9F6" />
              <p className="font-heading font-black text-4xl uppercase tracking-tighter">Spirited Lamb</p>
            </div>
            <p className="text-background/40 max-w-xs text-sm">Authentic Catholic community for young adults in Ventura County.</p>
          </div>
          <div className="flex gap-12">
            <div className="flex flex-col gap-4">
               ...
               <span className="text-sm font-bold hover:text-primary cursor-pointer">Facebook</span>
            ...
            <div className="flex flex-col gap-4">
               ...
               <span className="text-sm font-bold hover:text-primary cursor-pointer">Newsletter</span>
            </div>
          </div>
        </div>
