...
import { ArrowRight, Heart, Handshake, BookOpen, Play, Instagram, Mic2, ExternalLink, ShoppingCart, Sparkles, Star, Sun } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    const fetchReflection = async () => {
      ...
      try {
        ...
        const response = await ai.models.generateContent({
          ...
          contents: "Write a one-sentence, powerful spiritual intention for a Catholic Young Adult in Ventura County, California. Focus on coastal beauty or community growth.",
        });
  ...
  const eventTypes: (EventType | 'All')[] = ['All', 'Prayer', 'Social', 'Service', 'Study'];
  ...
  return (
    <Layout>
        ...
        <div className="absolute inset-0 z-0 bg-charcoal">
          {HERO_IMAGES.map((img, idx) => (
            ...
            >
              <img 
                src={img.url} 
                className={`w-full h-full object-cover brightness-[0.5] contrast-110 transition-transform duration-[4000ms] ease-out ${idx === activeMediaIndex ? 'scale-110' : 'scale-100'}`}
                alt={img.label}
              />
            </div>
          ))}
      ...
      {/* SECTION 2: CALENDAR */}
      <section id="events" className="py-24 md:py-32 px-6 md:px-12 max-w-[1400px] mx-auto space-y-16 md:space-y-24">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12">
          <div className="space-y-4">
            <span className="text-xs font-black uppercase tracking-[0.6em] text-gold">Ventura Pulse</span>
            <h2 className="font-heading text-6xl md:text-9xl font-black text-charcoal tracking-tighter leading-[0.8] uppercase">CALENDAR</h2>
          </div>
          
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap gap-3 p-2 bg-charcoal/5 rounded-[2.5rem] shadow-inner">
              {eventTypes.map((t) => (
                <button
                  key={t}
                  onClick={() => setActiveType(t)}
                  className={`px-6 md:px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${
          ...
          <div className="glass rounded-[3rem] md:rounded-[4.5rem] p-8 md:p-14 shadow-2xl border border-charcoal/5 relative group overflow-hidden">
            <div className="flex justify-between items-center mb-16 relative z-10">
              <div className="space-y-1">
                <h3 className="font-heading text-4xl md:text-5xl font-black text-charcoal tracking-tighter uppercase leading-none">{format(currentDate, 'MMMM')}</h3>
                <p className="text-[10px] font-black text-charcoal/30 tracking-[0.4em] uppercase text-center md:text-left">Ventura County Updates</p>
              </div>
              <div className="flex gap-4">
                <button onClick={() => setCurrentDate(addDays(currentDate, -30))} className="w-12 h-12 md:w-14 md:h-14 flex items-center justify-center border-2 border-charcoal/5 rounded-full hover:bg-primary hover:text-white transition-all shadow-sm">&larr;</button>
                <button onClick={() => setCurrentDate(addDays(currentDate, 30))} className="w-12 h-12 md:w-14 md:h-14 flex items-center justify-center border-2 border-charcoal/5 rounded-full hover:bg-primary hover:text-white transition-all shadow-sm">&rarr;</button>
              </div>
            </div>

            <div className="grid grid-cols-7 gap-2 md:gap-6 text-center mb-10">
              {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(d => (
                <span key={d} className="text-[9px] md:text-[10px] font-black text-charcoal/20 uppercase tracking-[0.2em] md:tracking-[0.3em]">{d}</span>
              ))}
            </div>
              ...
              {calendarDays.map((day, idx) => {
                ...
                return (
                  <div 
                    ...
                    className={`relative aspect-square flex items-center justify-center rounded-xl md:rounded-[1.8rem] text-sm font-black transition-all cursor-pointer group ${
                      !isCurrentMonth ? 'text-charcoal/5 opacity-0 pointer-events-none' : 'text-charcoal/80'
                    } ${
                  ...
                  >
                    {format(day, 'd')}
                    {dayEvents.length > 0 && (
                      <span className="absolute -top-1 -right-1 w-5 h-5 md:w-6 md:h-6 bg-primary text-white text-[9px] md:text-[10px] rounded-full flex items-center justify-center border-2 md:border-4 border-white font-black">
                        {dayEvents.length}
                      </span>
                    )}
                  </div>
      ...
      {/* SECTION 3: PARTNER */}
      <section id="partner" className="relative py-12 bg-charcoal text-white overflow-hidden border-y border-white/5">
        <div className="max-w-[1400px] mx-auto px-6 relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-12">
          <div className="max-w-xl text-center lg:text-left">
            <h2 className="font-heading text-3xl md:text-5xl font-black text-white tracking-tighter uppercase leading-[1]">
              VENTURA <span className="text-primary">PARTNERS</span>
            </h2>
            <p className="text-white/40 text-xs md:text-sm font-medium mt-2">Fueling faith across our local coastline.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
             <button className="px-8 py-4 bg-primary text-white rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl">Donate Now</button>
             <button className="px-8 py-4 bg-white text-charcoal rounded-3xl font-black text-xs uppercase tracking-widest shadow-lg">Be a Sponsor</button>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-12 max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 rounded-[3rem] md:rounded-[5rem] overflow-hidden shadow-2xl border border-charcoal/5 bg-charcoal">
          {[
            { title: 'Spirituality', desc: 'San Buenaventura Life', img: "https://images.unsplash.com/photo-1544427920-c49ccfb85579?auto=format&fit=crop&q=80&w=1200" },
            { title: 'Action', desc: 'Coastal Mission', img: "https://images.unsplash.com/photo-1551632811-561732d1e306?auto=format&fit=crop&q=80&w=800" },
            { title: 'Connection', desc: 'Local Fellowship', img: "https://images.unsplash.com/photo-1517486808906-6ca8b3f04846?auto=format&fit=crop&q=80&w=800" }
          ].map((card, idx) => (
            <div key={idx} className="group relative h-[500px] md:h-[600px] overflow-hidden cursor-pointer border-b md:border-b-0 md:border-r border-white/5">
              ...
              <div className="absolute bottom-10 left-10 text-white z-10">
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gold mb-2">{card.desc}</p>
                <h4 className="text-4xl md:text-5xl font-black uppercase tracking-tighter">{card.title}</h4>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="media" className="py-24 md:py-48 bg-charcoal text-white relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col lg:flex-row items-baseline justify-between gap-6 mb-24">
            <div className="space-y-4">
              <span className="text-xs font-black uppercase tracking-[0.8em] text-gold">Digital Fellowship</span>
              <h2 className="font-heading text-7xl md:text-[10rem] font-black tracking-tighter uppercase leading-[0.75] text-white">
                VENTURA <br /> <span className="text-primary">MEDIA</span>
              </h2>
            </div>
            <div className="max-w-md text-left lg:text-right">
               <p className="text-white/40 text-xl font-medium leading-relaxed italic">
                 Explore the voices and visions of our Ventura community across the digital landscape.
               </p>
            </div>
          </div>
            ...
            <div className="md:col-span-2 lg:col-span-3 bg-white/5 backdrop-blur-xl rounded-[3rem] p-10 border border-white/10 flex flex-col justify-between group overflow-hidden relative">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-[0.4em] text-primary">Ventura Intention</span>
                </div>
                <h3 className={`text-2xl md:text-4xl font-black leading-tight uppercase tracking-tighter transition-opacity duration-500 ${isAiLoading ? 'opacity-30' : 'opacity-100'}`}>
                   {aiReflection}
                </h3>
              </div>
              <div className="flex items-center gap-4">
                ...
                <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Ventura Community</p>
              </div>
            </div>

            {/* Podcast Card */}
            <div className="md:col-span-2 lg:col-span-3 bg-primary rounded-[3rem] p-10 flex flex-col justify-between group cursor-pointer hover:shadow-2xl hover:shadow-primary/20 transition-all">
              <div className="flex justify-between items-start">
                <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center">
                  <Mic2 size={32} />
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black uppercase tracking-widest mb-1">Coastal Faith</p>
                  <p className="text-xs font-bold text-white/70 italic">Ep. 12: Mission San Buenaventura</p>
                </div>
              </div>
              <div className="space-y-4">
                <h3 className="text-4xl font-black uppercase tracking-tighter leading-none">THE LAMB'S <br /> PATH</h3>
                <div className="flex items-center gap-4">
                   ...
                   <span className="text-[10px] font-black">18:14</span>
                </div>
              </div>
              <button className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.4em] hover:gap-5 transition-all">
                Listen on Spotify <ArrowRight size={16} />
              </button>
            </div>

            {/* YouTube Card */}
            <div className="md:col-span-4 lg:col-span-6 row-span-2 rounded-[3.5rem] overflow-hidden relative group cursor-pointer shadow-2xl">
              <img 
                ...
                alt="Ventura Sessions Video"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-charcoal via-charcoal/20 to-transparent opacity-80" />
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-28 h-28 bg-white/10 backdrop-blur-xl rounded-full flex items-center justify-center group-hover:scale-115 group-hover:bg-primary transition-all duration-700 shadow-[0_0_50px_rgba(192,90,63,0.3)]">
              ...
              <div className="absolute bottom-16 left-16 right-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
                 <div className="space-y-4">
                   <span className="text-xs font-black uppercase tracking-[0.6em] text-gold block">Sunday Sessions</span>
                   <h3 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-[0.85]">Live from <br /> our Coastal <br /> Mission</h3>
                 </div>
                 <button className="px-10 py-5 bg-white text-charcoal rounded-full font-black text-xs uppercase tracking-widest hover:bg-gold hover:text-white transition-all shadow-xl">
                    Ventura Playlist
                 </button>
              </div>
            </div>
            ...
            <div className="md:col-span-4 lg:col-span-6 border-t border-white/10 pt-16 mt-8 grid grid-cols-2 md:grid-cols-4 gap-6">
               <div className="col-span-2 flex flex-col justify-center">
                  <div className="flex items-center gap-5 mb-6">
                    ...
                    <h4 className="text-4xl font-black uppercase tracking-tighter">@SpiritedLamb</h4>
                  </div>
                  <p className="text-white/40 text-sm font-bold uppercase tracking-[0.2em] leading-relaxed max-w-sm">Connect for Ventura stories, local highlights, and updates.</p>
               </div>
               {[
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 mb-24">
            <div className="space-y-6">
              <span className="text-xs font-black uppercase tracking-[0.8em] text-primary">Limited Release</span>
              <h2 className="font-heading text-7xl md:text-[10rem] font-black text-charcoal tracking-tighter leading-[0.75] uppercase">
                THE <br /> SPIRITED <br /> <span className="text-gold">SHOP</span>
              </h2>
            </div>
            <div className="max-w-md text-right flex flex-col items-end gap-6">
               <p className="text-charcoal/50 text-xl font-medium leading-relaxed italic">
                 Every purchase fuels our Ventura community events and spiritual outreach.
               </p>
              ...
              <div key={item.id} className="group relative flex flex-col">
                <div className="relative aspect-[3/4] rounded-[3rem] overflow-hidden bg-charcoal/5 shadow-2xl transition-all duration-700 group-hover:-translate-y-4">
                  ...
                  <div className="absolute inset-0 bg-charcoal/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center">
                    <button className="bg-white text-charcoal px-10 py-5 rounded-full font-black text-[11px] uppercase tracking-widest shadow-2xl flex items-center gap-3 transform translate-y-6 group-hover:translate-y-0 transition-all">
                ...
                <div className="mt-10 px-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <h3 className="font-heading text-3xl font-black uppercase tracking-tight text-charcoal leading-none group-hover:text-primary transition-colors">
                      {item.name}
                    </h3>
                  ...
                  <div className="flex items-center gap-2">
                     ...
                     <p className="text-[11px] font-bold text-charcoal/40 uppercase tracking-[0.4em]">Ventura Mission Gear</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="relative z-10 bg-charcoal py-8 overflow-hidden">
          <div className="flex animate-scroll whitespace-nowrap items-center gap-16 w-max">
            {Array.from({length: 12}).map((_, i) => (
              <React.Fragment key={i}>
                <span className="text-4xl font-black tracking-tighter uppercase text-white/90">Ventura Faith</span>
                <Heart size={32} fill="#C05A3F" className="text-primary" />
                <span className="text-4xl font-black tracking-tighter uppercase text-gold">Community Joy</span>
